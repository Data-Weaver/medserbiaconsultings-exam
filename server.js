...
function shuffleAnswers(q) {
  ...
  options.forEach((opt, idx) => {
    if (opt.letter === q.correct_answer) newCorrect = newLetters[idx];
  });
  return {
    ...q,
...
app.post('/api/verify-code', (req, res) => {
  const { code } = req.body;
  const examCode = db.prepare("SELECT * FROM exam_codes WHERE code = ? AND used = 0 AND type = 'exam'").get(code);
  if (examCode) {
...
app.post('/api/submit-exam', (req, res) => {
  ...
  if (process.env.SMTP_USER && process.env.SMTP_PASS) {
    try {
      ...
      const pct = Math.round((totalCorrect / 80) * 100);
      const updatedSession = db.prepare('SELECT * FROM exam_sessions WHERE id = ?').get(sessionId);
      transporter.sendMail({
        from: process.env.SMTP_USER,
        to: process.env.ADMIN_EMAIL || process.env.SMTP_USER,
        subject: `📝 Νέο τεστ: ${updatedSession.full_name} — ${pct}%`,
        html: `<h2>Νέο τεστ ολοκληρώθηκε</h2>
          <p><strong>Όνομα:</strong> ${updatedSession.full_name}</p>
          <p><strong>Email:</strong> ${updatedSession.email}</p>
          <p><strong>Τηλέφωνο:</strong> ${updatedSession.phone}</p>
          <p><strong>Βαθμός:</strong> ${totalCorrect}/80 (${pct}%)</p>
          <p><strong>Βιολογία:</strong> ${biologyCorrect}/40</p>
          <p><strong>Χημεία:</strong> ${chemistryCorrect}/40</p>
          <p><a href="https://exam.medserbiaconsultings.gr/admin.html">Δείτε τα αποτελέσματα</a></p>`
      });
...
// Check if study is accessible (no active exam) + verify study code
app.post('/api/check-study-access', (req, res) => {
  const { code } = req.body;
  const ip = getClientIP(req);
  const activeExam = db.prepare('SELECT id FROM exam_sessions WHERE ip_address = ? AND status = ?').get(ip, 'in_progress');
  if (activeExam) {
  ...
  const studyCode = db.prepare("SELECT * FROM exam_codes WHERE code = ? AND used = 0 AND type = 'study'").get(code);
  if (studyCode) {
    db.prepare('UPDATE exam_codes SET used = 1, used_at = CURRENT_TIMESTAMP WHERE id = ?').run(studyCode.id);
    return res.json({ blocked: false, codeId: studyCode.id });
  }
  res.json({ blocked: true, reason: 'Λάθος ή χρησιμοποιημένος κωδικός.' });
});
...
// Study mode - get wrong answers for a user
app.get('/api/admin/study-wrong/:name', adminAuth, (req, res) => {
  const wrong = db.prepare(`
    SELECT sa.full_name, sa.email, sa.selected_answer, sa.category, q.question_text, q.correct_answer
    FROM study_answers sa JOIN questions q ON sa.question_id = q.id
    WHERE sa.full_name = ? AND sa.is_correct = 0
    ORDER BY sa.category, sa.id
  `).all(decodeURIComponent(req.params.name));
  res.json(wrong);
});
