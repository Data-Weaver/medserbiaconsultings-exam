...
db.exec("CREATE TABLE IF NOT EXISTS study_answers (id INTEGER PRIMARY KEY AUTOINCREMENT, full_name TEXT NOT NULL, question_id INTEGER NOT NULL, selected_answer TEXT NOT NULL, is_correct INTEGER NOT NULL, category TEXT NOT NULL, answered_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
db.exec("CREATE TABLE IF NOT EXISTS exam_codes (id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT NOT NULL UNIQUE, used INTEGER DEFAULT 0, used_by TEXT, used_at DATETIME)");
...
if (codeCount === 0) { var ins = db.prepare('INSERT INTO exam_codes (code) VALUES (?)'); for (var i = 0; i < 200; i++) { ins.run('MED-' + crypto.randomBytes(3).toString('hex').toUpperCase()); } console.log('Generated 200 codes'); }
